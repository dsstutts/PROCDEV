...
public class ArduinoConnection {
	...
	void connect(String portName) throws Exception {
		 ...
		 * 
		 * // The port must be closed before being accessed if (
		...
		public void serialEvent(/* SerialPortEvent arg0 */) {
			try {
				while ((data = (char) in.read()) > -1) {
					...
					if(data == ',' || data =='\r' || data == '\n') {
						if(debug) System.out.println("Switching state from " + state.toString());
						
						switch(state) {
						case SERIES:
							series = Integer.parseInt(buf.toString());
							state = State.TIMESTAMP;
							if(debug) System.out.println("to " + state.toString());
							if(debug) System.out.println("Read value " + series);
							break;
						case TIMESTAMP:
							timestamp = Integer.parseInt(buf.toString());
							state = State.DATA_POINT;
							if(debug) System.out.println("to " + state.toString());
							if(debug) System.out.println("Read value " + timestamp);
							break;
						case DATA_POINT:
							data_point = Double.parseDouble(buf.toString());
							state = State.SERIES;
							if(debug) System.out.println("to " + state.toString());
							if(debug) System.out.println("Read value " + data_point);
							break;
						case SERIES_NUM:
							series_num = Integer.parseInt(buf.toString());
							state = State.SERIES_NAME;
							if(debug) System.out.println("to " + state.toString());
							if(debug) System.out.println("Read value " + series_num);
							break;
						case SERIES_NAME:
							series_name = buf.toString();
							state = State.SERIES;
							if(debug) System.out.println("to " + state.toString());
							if(debug) System.out.println("Read value " + series_name);
							break;
						case SETTING_NUM:
							setting_num = Integer.parseInt(buf.toString());
							state = State.SETTING_NAME;
							if(debug) System.out.println("to " + state.toString());
							if(debug) System.out.println("Read value " + setting_num);
							break;
						case SETTING_NAME:
							setting_name = buf.toString();
							state = State.SETTING_TYPE;
							if(debug) System.out.println("to " + state.toString());
							if(debug) System.out.println("Read value " + setting_name);
							break;
						case SETTING_TYPE:
							setting_type = Integer.parseInt(buf.toString());
							state = State.SERIES;
							if(debug) System.out.println("to " + state.toString());
							if(debug) System.out.println("Read value " + setting_type);
							break;
						default:
							state = State.SERIES;
