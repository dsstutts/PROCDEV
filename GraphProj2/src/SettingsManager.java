...
public class SettingsManager extends JFrame implements PropertyChangeListener,
		ActionListener {
	...
	public SettingsManager() {
		...
		gridConstraints.anchor = GridBagConstraints.WEST; // Left align cell
															// contents
		...
		for (int i = 0; i < Constants.DEF_SETTINGSMANAGER_LABELS.length; i++) {
			...
			settingsManagerLabels[i] = new JLabel(
					Constants.DEF_SETTINGSMANAGER_LABELS[i]);
			settingsManagerLabels[i].setFont(labelFont);
			graphPanel.add(settingsManagerLabels[i], gridConstraints);

			// Add text fields
			gridConstraints.gridx = 1;
			settingsManagerFields[i] = new JFormattedTextField(
					Constants.DEF_SETTINGSMANAGER_FIELDS[i]);
			settingsManagerFields[i].setColumns(Constants.DEF_TEXTBOX_WIDTH); // Set
																				// textbox
																				// width
			settingsManagerFields[i].addPropertyChangeListener("value", this);

			graphPanel.add(settingsManagerFields[i], gridConstraints); // Add to
																		// graphPanel
		}

		// Add Time measurement dropdown menu (s,ms)
		gridConstraints.gridy++;
		gridConstraints.gridx = 0;
        ...
        timeMeasurement_Dropdown.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent event) {
                // Get the source of the component, which is our combo box.
                JComboBox timeMeasurement_Dropdown = (JComboBox) event.getSource();
		...
		seriesPanel
				.setBackground(Constants.DEF_SETTINGSMANAGER_BACKGROUNDCOLOR);

		// Add series title
		gridConstraints.anchor = GridBagConstraints.CENTER;
		gridConstraints.gridwidth = 3; // series Label will span 4 columns
		gridConstraints.gridx = 0;
		gridConstraints.gridy = 0;
		...
		/************************************ Here start Series1 Series 1 Box ToDo ******************/
		for (int i = 0; i < Constants.DEF_SERIES_TITLES.length; i++) {
			...
			seriesFields[i] = new JFormattedTextField(
					Constants.DEF_SERIES_TITLES[i]);
			seriesFields[i].setColumns(Constants.DEF_TEXTBOX_WIDTH);
		...
		settingsManagerPanel.setBackground(Color.PINK);
		...
		JLabel footer = new JLabel(
				"TODO: hide/show, Line style, add/remove series");
		settingsManagerPanel.add(footer, gridConstraints);

	}

	/************************************************************
	 * Name: actionPerformed Pre: The color chooser button was pressed for
	 * changing the color of a series Post: A new window appears allowing the
	 * user to select a precise color for their graph. If the color was changed,
	 * the button's color as well as the corresponding series values on the
	 * graph
	 *************************************************************/
	public void actionPerformed(ActionEvent event) {
	...
	public void propertyChange(PropertyChangeEvent e) {
		...
		// Change x axis label
			...
			ArduinoMain.myGraph.setXAxisLabel((String) settingsManagerFields[1]
					.getValue());
			System.out.println("Changed X Axis Label to "
					+ settingsManagerFields[1].getValue());
		}
		// Change y axis label
	...
	/************************************************************
	 * Name: createSettingsManager Pre: None Post: Creates a form to allow the
	 * user to change the graph settings
	 *************************************************************/
